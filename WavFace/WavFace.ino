// References: 
// http://makezine.com/2016/09/28/halloween-2016-7-ideas-for-automating-your-halloween-porch-display/
// https://facelesstech.wordpress.com/2015/10/31/atmega_eyes-animated-eyes-for-pumpkin/
// https://learn.adafruit.com/animating-multiple-led-backpacks/ideas?view=all#software
// https://learn.adafruit.com/animating-multiple-led-backpacks/ideas?view=all#wiring
//
...
static const uint8_t 
  myseq[][24] = {{ 0, 2,   2, 5,   5, 3,   3, 7, // "Beware, I live!"
                 4, 5,   3, 4,   2, 5,   4, 3,
                 3, 4,   1, 5,   3, 5,    255, 255 },
               { 0, 1,   3, 5,   1, 5,   4, 2, // "I hunger!"
                 3, 2,   1, 2,   4, 4,   1, 3,
                 4, 2,   255, 255, 255, 255, 255, 255 },
               { 0, 1,   1, 2,   3, 6,   2, 5, // "Run, coward!"
                 0, 1,   4, 4,   5, 2,   1, 5,
                 3, 6,   1, 4,    255, 255, 255, 255 }};

#define FRAMERATE       20
#define SLEEP_MINUTES    1
#define SLEEP_FRAMES     SLEEP_MINUTES * 60 * FRAMERATE

const uint8_t
  blinkIndex[] PROGMEM = { 1, 2, 3, 4, 3, 2, 1 }; // Blink bitmap sequence
  
int 
  lastMotionCountdown = SLEEP_FRAMES;
  
uint8_t
  inputDelayCountdown =   0, // Countdown to next input check (in frames)
...
void setup() {
  ...
  for(uint8_t i=6; i<=9; i++) {
    pinMode(i, INPUT);
    if ( i != 9 ) {  digitalWrite(i, HIGH); } // Enable pullup for buttons but not the PIR Sensor
  }
 pinMode( 7, OUTPUT);
  digitalWrite(7, HIGH);
  //uint8_t PIR_MOTION_SENSOR = 9;
...
void loop() {
  ...
  uint8_t i = scanButtons();

  if (lastMotionCountdown > 0) {
    ...
    if(i < 3) {   // Anything pressed?  Yes!
          ...
          newPos         = seq[idx++]; // Initial mouth pos
          mouthCountdown = seq[idx++]; // Hold time for pos
  ...
  if (lastMotionCountdown%100 == 0) {Serial.println(lastMotionCountdown);}
  if (lastMotionCountdown != 0) { --lastMotionCountdown; }
...
uint8_t scanButtons() {
  ...
  if (inputDelayCountdown <= 0 ) {
    // Scan buttons 6, 7, 8 & 9 looking for first button pressed...
   // for (i=0; (i<5) && (digitalRead(i+6) == HIGH); i++);
   i = random(0, 6); prevBtn= i;
   
    if(i < 4) {               // Anything pressed?  Yes!
       /* Serial.print("Button ");
          Serial.print(i);
          Serial.print(" prevBtn ");
          Serial.print(prevBtn);
          Serial.print(" btnCount ");
          Serial.println(btnCount); */
      if(i == prevBtn) {      // Same as last time we checked?  Good!
        
        if(++btnCount == 3) { // 3 passes to 'debounce' this input
          ...
          if (i == 3) {
            ...
            else if ( lastMotionCountdown <= SLEEP_FRAMES / 2) {
              action = random(3); 
      ...
      prevBtn = i;   // No buttons pressed
    }
 } else --inputDelayCountdown;
...
// -------------------------------------------------------------------
// allOff()    
// Turns off all LEDs - used during sleep mode
//
// -------------------------------------------------------------------
void allOff() {
